...
# Windows
   ...
   SDL_FRONTEND_LIBS := -lmingw32 -lSDL2main -lSDL2 -static -lwinmm -lole32 -limm32 -loleaut32 -lversion -lsetupapi
   SDL_FRONTEND_LIBS += $(shell pkg-config --libs-only-l sdl2)
   SDL_FRONTEND_LDFLAGS += $(shell pkg-config --libs-only-L --libs-only-other sdl2)
   SDL_FRONTEND_OBJECTS := libretro/frontend/sdlarch.o libretro/frontend/glad.o $(ROMDIR)/roms.o
...
all: $(TARGET) $(SDL_FRONTEND_TARGET)
...
ifeq ($(ROM_BUILD), 1)
ROMS = $(ROMDIR)/sonic1.md \
       $(ROMDIR)/sonic2.md \
       $(ROMDIR)/sonic3.md

$(ROMDIR)/sonic1.md: .FORCE
	cd roms/s1disasm; ./build.bat
	@mv roms/s1disasm/s1.bin '$@'

$(ROMDIR)/sonic2.md: .FORCE
	cd roms/s2disasm; ./build.bat
	@mv roms/s2disasm/s2built.md '$@'

$(ROMDIR)/sonic3.md: .FORCE
	cd roms/skdisasm; ./build.bat
	@mv roms/skdisasm/sonic3k.bin '$@'

$(ROMDIR)/roms.rc: roms/makeroms.sh
...
ifeq ($(SDL_FRONTEND), 1)
$(SDL_FRONTEND_TARGET): $(OBJECTS) $(SDL_FRONTEND_OBJECTS)
	$(LD) $(LINKOUT)$(SDL_FRONTEND_TARGET) $(fpic) $(OBJECTS) $(SDL_FRONTEND_OBJECTS) $(SDL_FRONTEND_LDFLAGS) $(SDL_FRONTEND_LIBS)
endif
